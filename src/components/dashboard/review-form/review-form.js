...
class ReviewForm extends Form {
	...
	render() {
		const { data, staffs, isToggled, customFieldForm, customFields, errors, isSubmitted, editMode } = this.state;
				...
				{!isSubmitted && (
					<div className="col-box">
						<div className="row">
							...
							<ImagePreview />
							<div className="col-12 text-center">
								<button className="btn btn-primary" onClick={this.toggleCustomField}>
						...
						<div className="row">
								...
								{isToggled && (
									<div className="col-12 mt-2">
										<form onSubmit={this.handleAddCustomField}>
											<input
												type="text"
												className="form-control toggleInfoForm"
												placeholder="Skill"
												required
												value={customFieldForm.fieldName}
												onChange={this.handleCustomFieldChange}
											/>
											<button className="btn btn-primary mt-2 col-3 toggleInfoForm">Add</button>
										</form>
									</div>
								)}
							</div>
							<div className="col">
								<form onSubmit={this.handleSubmit}>
									{this.renderSelect('staffId', 'Staff', staffs, data.staffId, editMode)}
									...
									{customFieldsArray.length > 0 && (
										<div className="RatingsDiv">
											...
											{customFieldsArray.map((field) => (
												<div key={field}>
													{field} &nbsp;
													<RatingStar className="ml-2" size={5} isClickable={false} />
													<i
